...
export const createNewMonthlySheet = async (userId, accountInformation) => {
  ...
  const monthAndYear =
    accountInformation.month === undefined
      ? dateSheet.toLocaleString('default', { month: 'long', year: 'numeric' })
      : accountInformation.month;
  const monthlyDocument = await firestore
    .collection('budgetTrackers')
    .doc(userId)
    .collection('monthlySheets')
    .doc(monthAndYear)
    .get();
